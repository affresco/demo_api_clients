...
# Local
from .identification import client_id
...
class deltaBaseClient(object):
    ...
    def __init__(self, key=None, secret=None):
        ...
        logging.debug(f"[{self.id}] delta alpha instance created.")
    ...
    def parse_key(cls, key=None):
        if not key:
            logger.warning("No API key provided to delta alpha.")
        return key

    @classmethod
    def parse_secret(cls, secret=None):
        if not secret:
            logger.warning("No API secret provided to delta alpha.")
        return secret
